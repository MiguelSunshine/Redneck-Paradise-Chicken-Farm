...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function closeModalDOM(instant = false) {
        ...
        if (allModals.length <= 1) {
            activeModalObjectUrls.forEach(url => URL.revokeObjectURL(url as string));
    ...
    function renderRingIndicator(animal: Animal) {
    ...
    async function renderSponsorModal(options: RenderSponsorModalOptions = {}) {
        ...
        const levelOrder: { [key: string]: number } = { "King Edition": 1, "Gold": 2, "Silber": 3 };
        ...
        openModal(content, modal => {
            const searchInput = modal.querySelector('#sponsor-search') as HTMLInputElement;
            const sortSelect = modal.querySelector('#sponsor-sort') as HTMLSelectElement;
            const levelFilterSelect = modal.querySelector('#sponsor-level-filter') as HTMLSelectElement;
            ...
            modal.querySelectorAll('.sponsor-list-card').forEach(c => c.addEventListener('click', (e) => renderSponsorDetailModal(parseInt((e.currentTarget as HTMLElement).dataset.id))));
        });
    }
    ...
    function handleEggFormSubmit(e: Event) {
        e.preventDefault();
        const chickenCount = parseInt((document.getElementById('chicken-eggs') as HTMLInputElement).value) || 0;
        const duckCount = parseInt((document.getElementById('duck-eggs') as HTMLInputElement).value) || 0;
    ...
    function importData(event: Event) {
        ...
        reader.onload = async e => {
            ...
            try {
                ...
                await Promise.all((importedAnimals as any[]).map(async (animal: any) => {
                ...
                await Promise.all((importedSponsors as any[]).map(async (sponsor: any) => {
                ...
                state.animals = importedAnimals as Animal[];
                state.sponsors = importedSponsors as Sponsor[];
                state.eggLogs = XLSX.utils.sheet_to_json(workbook.Sheets['Eier']) as EggLog[];
